...
#Filename: rscpuecache.java

#Author: Seth Roller

#Date: 4/25/19

#Problem: Will be implementing a 1K
cache with four bytes in of data in
each line
...
public class rscpuecache
{
    ...
    static cacheLines[] cache = new cacheLines[1024];
	...
	System.out.println();
	System.out.println("rscpuecache written by Seth Roller");
	System.out.println("Will be implementing a 1K"
			   + "cache with four bytes in of data in\n"
			   + "each line.");
	System.out.println();
    ...
    public static byte cacheread(short address) {
	
	byte expectedtag = (byte)((short)(address & (short)0xf000) >> 12);
	byte using = 0;

	System.out.println("Cacheread");
        System.out.printf("\tReferencing AR = %d\n",
				  (int)(address & (int)0xffff));
	System.out.printf("\tMain Memory Block = %d\n",
			  (int)((address / 4) & (int)0xffff));
	System.out.printf("\tCache Line = %d\n",
			  (int)(((address / 4) % 1024) & 0xffff));

	System.out.println("\tBefore testing data bytes and tag:");
	System.out.printf("\tData1 = %d Data2 = %d Data3 = %d Data4 = %d\n",
			  (int)(cache[(address / 4) % 1024].data1 & (int)0xff),
			  (int)(cache[(address / 4) % 1024].data2 & (int)0xff),
			  (int)(cache[(address / 4) % 1024].data3 & (int)0xff),
			  (int)(cache[(address / 4) % 1024].data4 & (int)0xff));
	System.out.printf("\tTag = %d\n", (int)(cache[(address / 4)
						  % 1024].tag & (int)0xff));
	
	if (cache[(address / 4) % 1024].tag != expectedtag) {
	    System.out.println("\tWe have a cache miss");
	    //read in next four bytes from memory
	    cache[(address / 4) % 1024].tag = expectedtag;
	    cache[(address / 4) % 1024].data1 = memory[address & 0xfffc];
	    cache[(address / 4) % 1024].data2 =	memory[(address & 0xfffc)
						       | 0x0001];
	    cache[(address / 4) % 1024].data3 =	memory[(address & 0xfffc)
						       | 0x0002];
	    cache[(address / 4) % 1024].data4 =	memory[(address & 0xfffc)
						       | 0x0003];


	    System.out.println("\tAfter testing data bytes and tag:");
	    System.out.printf("\tData1=%d  Data2=%d  Data3=%d "
			      + " Data4=%d\n",
			      (int)(cache[(address / 4) % 1024].data1
				    & (int)0xff),
			      (int)(cache[(address / 4) % 1024].data2
				    & (int)0xff),
			      (int)(cache[(address / 4) % 1024].data3
				    & (int)0xff),
			      (int)(cache[(address / 4) % 1024].data4
				    & (int)0xff));
	    System.out.printf("\tTag=%d\n", (int)(cache[(address / 4)
						      % 1024].tag & (int)0xff));

	    switch((byte)((short)0x0003 & address)) {
		
	    case (byte)0:
		using = cache[(address / 4) % 1024].data1;
		break;

	    case (byte)1:
		using = cache[(address / 4) % 1024].data2;
                break;

	    case (byte)2:
		using = cache[(address / 4) % 1024].data3;
                break;

	    case (byte)3:
		using = cache[(address / 4) % 1024].data4;
                break;
		...
		using = cache[(address / 4) % 1024].data4;
	     ...
	     //print out the line and tag following this
    ...
    public static void cachewrite(short address) {
	byte expectedtag = (byte)((short)(address & (short)0xf000) >> 12);

	System.out.println("Cachewrite");
	System.out.printf("\tReferencing AR = %d\n",
			  (int)(address & (int)0xffff));
	System.out.printf("\tMain Memory Block = %d\n",
			  (int)((address / 4) & (int)0xffff));
	...
	//if the cache does not have the memory upload to the cache
	//this includes the newly updated memory location
	//thus, I do everything in one step 
	if (cache[(address / 4) % 1024].tag != expectedtag) {
	...
	//if we have a hit then just edit the one byte of data
	//within the cache line
	    ...
	    case (byte)0:
		cache[(address / 4) % 1024].data1 = memory[address];
		break;

	    case (byte)1:
		cache[(address / 4) % 1024].data2 = memory[address];
                break;

	    case (byte)2:
		cache[(address / 4) % 1024].data3 = memory[address];
                break;

	    case (byte)3:
		cache[(address / 4) % 1024].data4 = memory[address];
                break;
	...
	//i update memory first then I write to the cache
	...
	//update main memory then update cache
	...
	System.out.println("INAC16 instruction");
	System.out.printf("AC = %d ZCVN = %d%d%d%d",
			  (int)(AC & (int)(0xffff)),
			  (byte)((flag & (byte)0x08) >> 3),
			  (byte)((flag & (byte)0x04) >> 2),
    ...
    private static class cacheLines {
